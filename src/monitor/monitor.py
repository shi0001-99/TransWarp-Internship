...
class StockAlert:
    ...
    def check_alerts(self, stock_config, data):
        ...
        【七大预警规则】
        1. 成本百分比（权重3）：盈利+15% / 亏损-12% → 止盈止损
        2. 固定价格（权重2）：突破/跌破指定价位
        3. 日内涨跌幅（权重1-3分级）：±4%，≥7%权重3，≥5%权重2
        4. 成交量异动（权重1-2）：放量≥2倍 / 缩量≤0.5倍
        5. 均线金叉/死叉（权重3）：MA5上穿/下穿MA10
        6. RSI超买/超卖（权重2）：RSI>70 / RSI<30
        7. 跳空缺口（权重2）：跳空≥0.5%
        8. 动态止盈（权重2-3）：盈利≥10%后回撤5%/10%
        
        【预警级别】（_calculate_alert_level）
        - critical（紧急）：总权重≥5 或 预警数≥3
        - warning（警告）：总权重≥3 或 预警数≥2
        - info（提醒）：单一低权重
        
        【防骚扰】（_alerted_recently）
        同类预警（同code+同atype）30分钟内只触发一次。
        """
        alerts = []
