...
class KellyCalculator:
    ...
    avg_win_pct = 0.15   # 平均盈利15%
    avg_loss_pct = 0.08  # 平均亏损8%
    → 盈亏比 b = 0.15/0.08 ≈ 1.875
    
    【半凯利安全边际】（H6硬约束）
    全凯利f*在数学上最优，但波动太大。
    半凯利f*/2牺牲20%收益，换取50%波动降低。
    kelly_scaling=0.5 实现半凯利缩放。
    
    【单票上限】（H9硬约束）
    single_max_fraction=0.25，任何单只股票仓位不超过25%。
    在半凯利之后再次min()截断。
    """
    
    @staticmethod
    def score_to_win_probability(score: float) -> float:
        """将评分(0-10)线性映射为胜率(35%-75%)
        
        评分0 → 胜率35%（即使最差也有35%胜率，避免过度悲观）
        评分10 → 胜率75%（即使最好也只有75%胜率，避免过度乐观）
        斜率 = 0.04/分，即每多1分胜率增加4%
        """
    ...
    def kelly_fraction(win_prob: float, avg_win_pct: float = 0.15, avg_loss_pct: float = 0.08) -> float:
        ...
        返回值 f ∈ [0, 1]：
        - f=0 表示不该下注（胜率太低或盈亏比太差）
        - f=1 表示全仓（胜率极高且盈亏比极佳）
        - 负值被截断为0（不允许做空）
        """
        w = win_prob           # 胜率 p
        l = 1.0 - w            # 败率 q = 1-p
        b = avg_win_pct / avg_loss_pct  # 盈亏比 b
    ...
    def calculate_position(score: float, total_capital: float = 1000000, 
        ...
        6. 建议股数 = 234534/34.5/100*100 = 6700股（向下取整到100股一手）
        """
        # Step 1: 评分转胜率（线性映射 0-10 → 35%-75%）
        win_prob = KellyCalculator.score_to_win_probability(score)
        
        # Step 2: 凯利公式计算最优仓位比例
        kelly_f = KellyCalculator.kelly_fraction(
            win_prob, 
            KELLY_CONFIG['avg_win_pct'],   # 0.15 平均盈利15%
            KELLY_CONFIG['avg_loss_pct']   # 0.08 平均亏损8%
        )
        
        # Step 3: 半凯利缩放（H6硬约束：kelly_scaling=0.5）
        # 全凯利波动太大，半凯利提供安全边际
        suggested_fraction = kelly_f * kelly_scaling
        
        # Step 4: 单票25%上限（H9硬约束）
        # 无论凯利公式算出多少，单只股票仓位不超过25%
        ...
        # Step 6: 计算建议股数（A股100股为一手，向下取整）
        suggested_shares = 0
        if current_price > 0:
            suggested_shares = int(suggested_amount / current_price / 100) * 100
        
        # edge = 原始凯利比例（未缩放），表示"优势"大小
        edge = kelly_f
        
        return {
            "score": score,
            "win_probability": round(win_prob, 4),        # 胜率
            "kelly_fraction": round(kelly_f, 4),           # 原始凯利比例（未缩放）
            "kelly_scaling": kelly_scaling,                # 缩放系数（0.5=半凯利）
            "suggested_fraction": round(suggested_fraction, 4),  # 最终建议仓位
            "suggested_amount": round(suggested_amount, 2),      # 建议金额
            "suggested_shares": suggested_shares,                # 建议股数
            "edge": round(edge, 4),                        # 优势
            "avg_win_pct": KELLY_CONFIG['avg_win_pct'],    # 平均盈利15%
            "avg_loss_pct": KELLY_CONFIG['avg_loss_pct'],  # 平均亏损8%
        }
